    ...
    class QuakeMap
    {
        ...
        public static Bitmap Draw()
        {
            ...
            epicenter = ToPixelCoordinate(new[] {
                InformationsChecker.LatestInformation.Location.Latitude,
                InformationsChecker.LatestInformation.Location.Longitude
            });
            //var areaInt = new Dictionary<int, string>();

            var cityInt = InformationsChecker.LatestInformation.Shindo.SelectMany(x => x.Place.SelectMany(y => y.Place.Select(z => new {
                    Place = z,
            ...
            cityInt = cityInt.OrderBy(x => x.Value).ToDictionary(x => x.Key, x => x.Value);
